...
`).join(""),n=document.querySelector(".gallery"),b=document.querySelector(".search-form"),c=document.querySelector(".loader"),d=document.querySelector(".load-more-btn");let l="",r=1;async function w(s){if(s.preventDefault(),l=s.target.elements.searchQuery.value.trim(),l===""){n.innerHTML="",s.target.reset(),u.warning({position:"topRight",transitionIn:"bounceInLeft",message:"input field cannot be empty",messageSize:16,timeout:3e3});return}n.innerHTML="",r=1,d.classList.add("is-hidden"),c.classList.remove("is-hidden");try{const e=await h(l,r);e.total===0?(s.target.reset(),u.error({position:"topRight",transitionIn:"bounceInLeft",message:"Sorry, there are no images matching your search query. Please try again!",messageSize:16,timeout:3e3})):(n.innerHTML=f(e.hits),new m(".gallery a",{captionsData:"alt",captionDelay:250}).refresh(),e.totalHits>r*15&&d.classList.remove("is-hidden"))}catch(e){console.log(e)}finally{s.target.reset(),c.classList.add("is-hidden")}}async function S(){r+=1,c.classList.remove("is-hidden");try{const s=n.firstElementChild.getBoundingClientRect().height,e=await h(l,r);n.insertAdjacentHTML("beforeend",f(e.hits)),new m(".gallery a",{captionsData:"alt",captionDelay:250}).refresh(),window.scrollBy({top:s*2,behavior:"smooth"}),e.totalHits<=r*15&&d.classList.add("is-hidden")}catch(s){console.log(s)}finally{c.classList.add("is-hidden")}}b.addEventListener("submit",w);d.addEventListener("click",S);
